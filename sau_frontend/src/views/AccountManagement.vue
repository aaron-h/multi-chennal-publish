...
const handleDownloadCookie = (row) => {
  ...
  const downloadUrl = `${baseUrl}/downloadCookie?filePath=${encodeURIComponent(row.filePath)}&token=${token}`
...
const connectSSE = (platform, name) => {
  ...
  const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5409'
  const token = encodeURIComponent(localStorage.getItem('token') || '')
  const url = `${baseUrl}/login?type=${type}&id=${encodeURIComponent(name)}&token=${token}`
