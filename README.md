# multi-channels-publish（万渠推）

`multi-channels-publish`（万渠推）是一个强大的自动化工具，旨在帮助内容创作者和运营者高效地将视频内容一键发布到多个国内外主流社交媒体平台。
...
1.  **克隆项目**:
    ```bash
    git clone https://github.com/dreammis/multi-channels-publish.git
    cd multi-channels-publish
    ```

2.  **安装依赖**:
    建议在虚拟环境中安装依赖。
    ```bash
    conda create -n multi-channels-publish python=3.10
    conda activate multi-channels-publish
    # 挂载清华镜像 or 命令行代理
...
1. **构建Docker镜像**:
    ```
   docker build -t multi-channels-publish:latest .
   ```
2. **运行Docker容器**:
    ```
   docker run -d -it -p 5409:5409 multi-channels-publish:latest
   ```
### 使用预构建镜像
1. **拉取镜像**:
    ```
   docker pull gzxy/multi-channels-publish:latest
   ```
2. **运行Docker容器**:
    ```
   docker run -d -it -p 5409:5409 gzxy/multi-channels-publish:latest
   ```
...
更详细的文档和说明，请查看：[multi-channels-publish 官方文档](https://sap-doc.nasdaddy.com/)
...
<table>
    <td align="center">
        ...
        <br />
        <a href="https://github.com/dreammis/multi-channels-publish/commits?author=dreammis" title="Code">💻</a>
        <br />
...
<table>
    <td align="center">
        ...
        <br />
        <a href="https://github.com/dreammis/multi-channels-publish/commits?author=LeeDebug" title="Code">💻</a>
...
[![Star History Chart](https://api.star-history.com/svg?repos=dreammis/multi-channels-publish&type=Date)](https://star-history.com/#dreammis/multi-channels-publish&Date)
